...
export const fetchInstructorPayments = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await api.get(
        `/finance/instructor/my-earnings?month=${month}&year=${year}`,
        { withCredentials: true }
      );
      return response.data; // Expecting { success: true, summary: {...}, paymentStatus: ... }
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
  }
...
export const fetchPlannedHoursReport = createAsyncThunk(
  "instructors/fetchPlannedHoursReport",
  async (params, { rejectWithValue }) => {
    try {
      const { instructorId, startDate, endDate } = params;
      const query = new URLSearchParams({ startDate, endDate });
      if (instructorId) query.append("instructor_id", instructorId);

      const response = await api.get(
        `/batch/reports/planned-hours?${query.toString()}`,
        { withCredentials: true }
      );
      return response.data;
    } catch (error) {
...
const InstructorSlice = createSlice({
  ...
  initialState: {
    ...
    plannedHoursReport: null,
    plannedHoursLoading: false,
    plannedHoursError: null,
  },
  reducers: {},
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchPlannedHoursReport.pending, (state) => {
        state.plannedHoursLoading = true;
        state.plannedHoursError = null;
      })
      .addCase(fetchPlannedHoursReport.fulfilled, (state, action) => {
        state.plannedHoursLoading = false;
        state.plannedHoursReport = action.payload.data;
      })
      .addCase(fetchPlannedHoursReport.rejected, (state, action) => {
        state.plannedHoursLoading = false;
        state.plannedHoursError = action.payload || 'Failed to fetch planned hours report';
      });
  },
}
);
