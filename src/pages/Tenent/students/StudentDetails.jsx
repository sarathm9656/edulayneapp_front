...
const StudentDetails = () => {
    ...
    const learningActivity = currentStudent?.learningActivity || {
        weeklyMinutes: 0,
        previousWeeklyMinutes: 0,
        trendPercent: 0,
        dailyActivityMinutes: [0, 0, 0, 0, 0, 0, 0]
    };
    const performance = currentStudent?.performance || {
        avgScore: 0,
        attendanceRate: 0,
        assignmentRate: 0,
        quizPassRate: 0
    };
    const weeklyMinutes = Number(learningActivity.weeklyMinutes || 0);
    const weeklyHoursPart = Math.floor(weeklyMinutes / 60);
    const weeklyMinutePart = weeklyMinutes % 60;
    const trendPercent = Number(learningActivity.trendPercent || 0);
    const trendPositive = trendPercent >= 0;
    const activityMinutes = Array.isArray(learningActivity.dailyActivityMinutes) && learningActivity.dailyActivityMinutes.length === 7
        ? learningActivity.dailyActivityMinutes
        : [0, 0, 0, 0, 0, 0, 0];
    const maxActivityMinutes = Math.max(...activityMinutes, 1);
    const dayLabels = Array.from({ length: 7 }, (_, i) =>
        new Date(Date.now() - (6 - i) * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", { weekday: "short" }).charAt(0)
    );
    const avgScore = Number(performance.avgScore || 0);
    const attendanceRate = Number(performance.attendanceRate || 0);
    const assignmentRate = Number(performance.assignmentRate || 0);
    const quizPassRate = Number(performance.quizPassRate || 0);
    const progressRingCircumference = 2 * Math.PI * 40;
    const avgScoreArc = (Math.min(Math.max(avgScore, 0), 100) / 100) * progressRingCircumference;
                        ...
                        <div className="col-md-7">
                            <div className="modern-card h-100 shadow-sm">
                                <div className="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <div className="badge bg-light text-muted fw-normal">Login-based (This Week)</div>
                                </div>
                                <div className="mb-4">
                                    <h2 className="mb-0 fw-bold">
                                        {weeklyHoursPart} <small className="fs-6 text-muted fw-normal">hours</small> {weeklyMinutePart} <small className="fs-6 text-muted fw-normal">minutes</small>
                                    </h2>
                                    <small className={trendPositive ? "text-success" : "text-danger"}>
                                        <i className={`fa-solid ${trendPositive ? "fa-arrow-up" : "fa-arrow-down"} me-1`}></i>
                                        {Math.abs(trendPercent)}% from last week
                                    </small>
                                </div>
                                <div className="d-flex align-items-end justify-content-between h-40 gap-2 mb-3" style={{ height: '150px' }}>
                                    {activityMinutes.map((minutes, i) => {
                                        const heightPercent = (minutes / maxActivityMinutes) * 100;
                                        return (
                                            <div key={i} className="flex-1 rounded-top" style={{ height: `${Math.max(heightPercent, 5)}%`, backgroundColor: '#ffeff6', position: 'relative', width: '100%', maxWidth: '35px' }} title={`${minutes} mins`}>
                                                <div className="rounded-top" style={{ height: '100%', width: '100%', backgroundColor: '#ff4d94', position: 'absolute', bottom: 0 }}></div>
                                            </div>
                                        );
                                    })}
                                </div>
                                <div className="d-flex justify-content-between text-muted small px-1">
                                    {dayLabels.map((label, idx) => <span key={idx}>{label}</span>)}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="modern-card h-100 shadow-sm">
                                ...
                                <div className="text-center py-2">
                                    <div className="position-relative d-inline-block">
                                        <svg width="140" height="140" viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)' }}>
                                            ...
                                            <circle cx="50" cy="50" r="40" fill="none" stroke="#ff4d94" strokeWidth="8" strokeDasharray={`${avgScoreArc} ${progressRingCircumference}`} />
                                        </svg>
                                        <div className="position-absolute top-50 start-50 translate-middle" style={{ transform: 'translate(-50%, -50%) rotate(0deg)' }}>
                                            ...
                                            <h3 className="mb-0 fw-bold">{avgScore}%</h3>
                                        </div>
                                    </div>
                                </div>
                                <div className="mt-4 space-y-3">
                                    <div className="d-flex justify-content-between align-items-center">
                                        ...
                                        <span className="fw-bold small">{attendanceRate}%</span>
                                    </div>
                                    <div className="d-flex justify-content-between align-items-center">
                                        <div className="d-flex align-items-center gap-2 small">
                                        ...
                                        <span className="fw-bold small">{assignmentRate}%</span>
                                    </div>
                                    <div className="d-flex justify-content-between align-items-center">
                                        <div className="d-flex align-items-center gap-2 small">
                                        ...
                                        <span className="fw-bold small">{quizPassRate}%</span>
                                    </div>
                                </div>
                            </div>
                        </div>
