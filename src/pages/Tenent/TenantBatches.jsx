...
const TenantBatches = () => {
  ...
  const fetchRecordings = async (batchId) => {
    ...
    try {
      const response = await api.get(`/dyte/recordings/${batchId}?include_hidden=true`);
  ...
  const handleDownloadRecording = (rec) => {
    ...
    if (!downloadUrl) {
      toast("Download link is not ready yet.");
      return;
    }
  ...
  const handleEditRecordingTitle = async (rec) => {
    if (!selectedBatchForRecordings?._id || !rec?.id || !rec?.type) return;
    const currentTitle = rec?.title || "";
    const newTitle = window.prompt("Enter new recording title", currentTitle);
    ...
    const actionKey = `edit:${rec.type}:${rec.id}`;
    setRecordingActionKey(actionKey);
    const toastId = toast.loading("Updating title...");
    try {
      const res = await api.patch(
        `/dyte/recordings/${selectedBatchForRecordings._id}/${rec.type}/${rec.id}`,
        { title: trimmed }
      );
      if (res.data?.success) {
        toast.success("Title updated", { id: toastId });
        await fetchRecordings(selectedBatchForRecordings._id);
      } else {
        toast.error(res.data?.message || "Failed to update title", { id: toastId });
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || "Failed to update title", { id: toastId });
    } finally {
      setRecordingActionKey("");
    }
  };

  const handleToggleRecordingVisibility = async (rec) => {
    if (!selectedBatchForRecordings?._id || !rec?.id || !rec?.type) return;
    const nextHidden = !Boolean(rec?.is_hidden);
    const actionLabel = nextHidden ? "Hiding..." : "Unhiding...";
    const actionKey = `hide:${rec.type}:${rec.id}`;
    setRecordingActionKey(actionKey);
    ...
    try {
      ...
      if (res.data?.success) {
        toast.success(nextHidden ? "Recording hidden" : "Recording visible", { id: toastId });
        await fetchRecordings(selectedBatchForRecordings._id);
      } else {
        toast.error(res.data?.message || "Failed to update visibility", { id: toastId });
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || "Failed to update visibility", { id: toastId });
    } finally {
      setRecordingActionKey("");
    }
  };
  ...
  const handleDeleteRecording = async (rec) => {
    if (!selectedBatchForRecordings?._id || !rec?.id || !rec?.type) return;
    const confirmDelete = window.confirm("Delete this recording? This action cannot be undone.");
    if (!confirmDelete) return;

    const actionKey = `delete:${rec.type}:${rec.id}`;
    setRecordingActionKey(actionKey);
    const toastId = toast.loading("Deleting recording...");
    try {
      const res = await api.delete(`/dyte/recordings/${selectedBatchForRecordings._id}/${rec.type}/${rec.id}`);
      if (res.data?.success) {
        toast.success("Recording deleted", { id: toastId });
        if (activeRecording?.id === rec.id && activeRecording?.type === rec.type) {
          setActiveRecording(null);
        }
        await fetchRecordings(selectedBatchForRecordings._id);
      } else {
        toast.error(res.data?.message || "Failed to delete recording", { id: toastId });
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || "Failed to delete recording", { id: toastId });
    } finally {
      setRecordingActionKey("");
    }
  };
  ...
  const renderStartButton = (batch) => {
    ...
    const strictEnabled = batch.is_strict_schedule !== false;
    if (strictEnabled) {
      if (!isToday) {
        return <span className="badge bg-light text-muted border">Not Today</span>;
      }
      if (!isWithinTime) {
        return <span className="badge bg-warning text-dark">Starts Soon</span>;
      }
    }
    ...
    // Strict mode validations are enforced in backend too; this UI is a quick hint.
    return (
                                ...
                                {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map(day => (
                                  <div key={day} className="form-check">
                                    <input
                                      ...
                                      type="checkbox"
                                      id={`day_${day}`}
                                      checked={form.recurring_days?.includes(day) || false}
                          ...
                          <div className="row mb-3">
                            ...
                            <div className="col-md-6">
                              ...
                              >
                                ...
                                {allCourseNames &&
                                  allCourseNames
                                    .filter((c) => (c.is_active && !c.is_archived) || c.id === editForm.course_id)
                                    .map((course) => (
                                      <option key={course.id} value={course.id}>
                                        {course.course_title}
                                      </option>
                                ...
                                {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map(day => (
                                  <div key={day} className="form-check">
                                    <input
                                      ...
                                      type="checkbox"
                                      id={`edit_day_${day}`}
                                      checked={editForm.recurring_days?.includes(day) || false}
                          ...
                          <div className="border rounded-3 p-3 mb-4 bg-light">
                            <div className="d-flex justify-content-between align-items-start gap-2 mb-3">
                              ...
                              <div className="d-flex gap-2 flex-wrap justify-content-end">
                                ...
                                >
                                  <i className="fa-solid fa-up-right-from-square me-1"></i> Open
                                </button>
                                <button
                            ...
                            <table className="table table-hover align-middle">
                              <thead className="table-light">
                                <tr>
                                  ...
                                  <th>Title / Date</th>
                                  <th>Duration</th>
                                  <th className="text-end">Access & Manage</th>
                                </tr>
                              </thead>
                              <tbody>
                                {recordings.map((rec) => (
                                  <tr key={`${rec.type}-${rec.id}`} className={rec.is_hidden ? "table-secondary" : ""}>
                                    <td>
                                    ...
                                    <td>
                                      <div className="d-flex flex-column">
                                        ...
                                        {rec.is_hidden && (
                                          <span className="badge bg-secondary mt-1 align-self-start">Hidden</span>
                                        )}
                                      </div>
                                    </td>
                                    <td className="small">
                                      ...
                                      <div className="d-flex justify-content-end gap-2 flex-wrap">
                                        {resolveRecordingLinks(rec).viewUrl && (
                                          <button
                                            ...
                                            disabled={recordingActionKey.endsWith(`${rec.type}:${rec.id}`)}
                                          >
                                            <i className="fa-solid fa-play me-1"></i> View
                                          </button>
                                        )}
                                        {resolveRecordingLinks(rec).downloadUrl && (
                                          <button
                                            type="button"
                                            className="btn btn-sm btn-outline-success rounded-pill px-3"
                                        ...
                                        <button
                                          type="button"
                                          className="btn btn-sm btn-outline-secondary rounded-pill px-3"
                                          onClick={() => handleEditRecordingTitle(rec)}
                                          disabled={recordingActionKey.endsWith(`${rec.type}:${rec.id}`)}
                                        >
                                        ...
                                        <button
                                          type="button"
                                          className="btn btn-sm btn-outline-dark rounded-pill px-3"
                                          onClick={() => handleToggleRecordingVisibility(rec)}
                                          disabled={recordingActionKey.endsWith(`${rec.type}:${rec.id}`)}
                                        >
                                          <i className={`fa-solid ${rec.is_hidden ? 'fa-eye' : 'fa-eye-slash'} me-1`}></i>
                                          {rec.is_hidden ? "Unhide" : "Hide"}
                                        </button>
                                        <button
                                          type="button"
                                        ...
                                        {!resolveRecordingLinks(rec).viewUrl && (
                                          <span className="badge bg-warning text-dark align-self-center">{rec.status}</span>
                                        )}
                                      </div>
                                    </td>
                                  </tr>
