...
const InstructorBatches = () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (authToken) {
                    ...
                    const url = `${window.location.origin}/meeting?authToken=${encodeURIComponent(authToken)}&role=${encodeURIComponent(role)}&batchId=${encodeURIComponent(batchId)}&meetingId=${encodeURIComponent(meetingId)}`;
    ...
    const handleJoinClass = (batchId, authToken) => {
        const url = `${window.location.origin}/meeting?authToken=${encodeURIComponent(authToken)}&role=instructor&batchId=${encodeURIComponent(batchId)}`;
        window.open(url, 'DyteMeetingWindow');
    };
                                                                        ...
                                                                        <td className="text-end">
                                                                            <div className="d-flex justify-content-end gap-2">
                                                                                ...
                                                                                {resolveRecordingLinks(rec).viewUrl && (
                                                                                    ...
                                                                                    >
                                                                                        <FaPlay className="me-1" style={{ fontSize: '9px' }} /> View
                                                                                    </button>
                                                                                )}
                                                                                ...
                                                                                {resolveRecordingLinks(rec).downloadUrl && (
                                                                                    ...
                                                                                    >
                                                                                        <i className="fa-solid fa-download me-1" style={{ fontSize: '10px' }}></i> Download
                                                                                    </button>
                                                                                )}

                                                                                {/* Status fallback if no URL */}
                                                                                {(!resolveRecordingLinks(rec).viewUrl && rec.status !== 'AVAILABLE') && (
